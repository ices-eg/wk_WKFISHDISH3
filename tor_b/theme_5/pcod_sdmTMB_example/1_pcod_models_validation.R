...
from_wd_to_qmd_reports_templates <- paste0(src_folder_location, "qmd_reports_templates/")
...
mean_nn2_dists <- data.frame(startyear = years,
                             mu_nn2 = 0,
                             sd_nn2 = 0)
...
base_grid <- base_grid %>% mutate(
  lon = coords_4326$X,
  lat = coords_4326$Y
)

base_grid_name <- "pcod_base_grid_"
saveRDS(base_grid, file = paste0("_data/", base_grid_name, grid_dist, "_km.rds"))
...
predgrid <- replicate_df(base_grid, "startyear", unique(df_surv$startyear)) %>%
  mutate(fYear=as.factor(startyear)) %>% st_drop_geometry()

#' Wrapper function to fit sdmtmb, predict and obtain index

sdm_fit(id = model_name,
        df = df_surv,
        meshcutoff = 10,
...
saveRDS(get(model_name), file = paste0("_saved_models/", data_name, "_", model_name, ".rds"))
assign(paste0(data_name, "_" ,model_name), readRDS(paste0("_saved_models/", data_name, "_", model_name, ".rds")))

# Model Report and Diagnostics
model_full_name <- paste0(data_name, "_", model_name)
current_wd <- getwd()
out_dir_report <- file.path(current_wd, from_wd_to_reports, data_name, "/Models/", model_full_name)
...
model_full_name <- paste0(data_name, "_", model_name)
current_wd <- getwd()
out_dir_report <- file.path(current_wd, from_wd_to_reports, data_name, "/Preds/", model_full_name)

grid_dist <- 2

quarto_render(
  input = paste0(from_wd_to_qmd_reports_templates, "Predictions_sdmTMB_report.qmd"),
  execute_dir = current_wd,
  execute_params = list(
    model_name = model_full_name,
    m_file = paste0("_saved_models/", model_full_name, ".rds"),
...
cross_validate_sdmTMB(model_files = model_paths, 
...
out_dir_report <- file.path(current_wd, from_wd_to_reports, data_name,
                            "Compare", comp_report_name)
dir.create(out_dir_report)

quarto_render(
  input = paste0(from_wd_to_qmd_reports_templates,"Comp_sdmTMB_models_report.qmd"),
  execute_dir = current_wd,
  execute_params = list(
...
sf_surv_m <- st_as_sf(df_surv, coords = c("longitudestart", "latitudestart"), crs = 4326) %>%
